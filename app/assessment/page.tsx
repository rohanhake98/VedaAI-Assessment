  ...
  return (
    <div className="flex h-screen overflow-hidden">
      ...
      <div className="flex flex-col flex-1 overflow-hidden">
        ...
        {/* Real metadata & AI grading metrics banner */}
        <div className="flex flex-wrap items-center justify-between gap-4 px-5 py-2.5 bg-gradient-to-r from-orange-50/90 via-white to-emerald-50/90 border-b border-gray-200 text-xs">
            ...
            {gradingResult ? (
              <>
                <div className="flex items-center gap-2 bg-white px-3 py-1 rounded-full border border-orange-200 shadow-sm">
                  <span className="font-bold text-[#E85D27]">Overall Score:</span>
                  <span className="font-extrabold text-sm text-gray-900">
                    {gradingResult.summary.totalMarksAwarded}
                    {gradingResult.summary.totalMaxMarks ? ` / ${gradingResult.summary.totalMaxMarks}` : ""}
                  </span>
                  {gradingResult.summary.percentage !== null && (
                    <span className="bg-emerald-100 text-emerald-800 text-[11px] font-bold px-1.5 py-0.2 rounded">
                      {gradingResult.summary.percentage}%
                    </span>
                  )}
                </div>

                <span className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full bg-emerald-100 text-emerald-800 font-bold border border-emerald-200 shadow-sm">
                  <span>✦</span> {gradingResult.summary.answeredCount}/{gradingResult.summary.totalQuestions} Graded
                </span>

                {gradingResult.summary.unansweredCount > 0 && (
                  <span className="inline-flex items-center gap-1 px-2 py-1 rounded-full bg-gray-100 text-gray-600 font-medium border border-gray-200">
                    {gradingResult.summary.unansweredCount} Unanswered
                  </span>
                )}

                {gradingResult.summary.ambiguousCount > 0 && (
                  <span className="inline-flex items-center gap-1 px-2 py-1 rounded-full bg-amber-100 text-amber-800 font-medium border border-amber-200">
                    {gradingResult.summary.ambiguousCount} Needs Review
                  </span>
                )}

                {gradingResult.summary.unmatchedAnswerCount > 0 && (
                  <span className="inline-flex items-center gap-1 px-2 py-1 rounded-full bg-purple-100 text-purple-800 font-medium border border-purple-200">
                    {gradingResult.summary.unmatchedAnswerCount} Unmatched Answers
                  </span>
                )}
              </>
            ) : hasRealMapping ? (
              ...
              <span className="inline-flex items-center gap-1.5 px-2.5 py-0.5 rounded-full bg-gray-100 text-gray-700 font-medium border border-gray-200">
                Demo Assessment Data (Upload files to run Real AI Grading)
              </span>
            )}
          ...
          <div className="flex items-center gap-2 text-gray-500">
            <span className="bg-emerald-50 text-emerald-700 border border-emerald-200 px-2 py-0.5 rounded text-[11px] font-semibold">
              Phase 8: AI Grading & Deterministic Scoring Active
            </span>
          </div>
        </div>
        ...
        <div className="flex flex-1 overflow-hidden">
          {/* Left: Question List & Grading Cards */}
          <div className="w-[520px] min-w-[360px] max-w-[560px] flex-shrink-0 border-r border-gray-200 overflow-hidden flex flex-col bg-white">
            <QuestionList
              ...
              onUpdateTeacherMarks={updateTeacherMarks}
            />
          </div>
